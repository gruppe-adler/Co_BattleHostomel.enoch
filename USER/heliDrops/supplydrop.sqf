/*
    File: supplydrop.sqf
    Author: Vilem

    Description:
    Script for para-drop of objects. Spawns waitUntil that handles ground hit (detaching of object from parachute). Used by supplydrop service.

    Parameter(s):
    1: <object> air unit
    2: <string> object class that will be dropped (use: "reammobox" or "land" for dropping ammobox/car appropriate to side of air vehicle)
    
    Returns:
    N/A
*/



/////* SMOKE*/////

/////* SMOKE-END*/////


KK_fnc_paraDrop = {
        private ["_class","_para","_paras","_p","_veh","_vel","_time"];
        _class = "O_Parachute_02_F";
        _para = createVehicle [_class, [0,0,0], [], 0, "FLY"];
        _para setDir getDir _this;
        _para setPos getPos _this;
        _paras =  [_para];
        _this attachTo [_para, [0,2,0]];
        {
            _p = createVehicle [_class, [0,0,0], [], 0, "FLY"];
            _paras set [count _paras, _p];
            _p attachTo [_para, [0,0,0]];
            _p setVectorUp _x;
        } count [
            [0.5,0.4,0.6],[-0.5,0.4,0.6],[0.5,-0.4,0.6],[-0.5,-0.4,0.6]
        ];
        0 = [_this, _paras] spawn {
            _veh = _this select 0;
            waitUntil {getPos _veh select 2 < 4};
            _vel = velocity _veh;
            detach _veh;
            _veh setVelocity _vel;
            playSound3D [
                "a3\sounds_f\weapons\Flare_Gun\flaregun_1_shoot.wss",
                _veh
            ];
            {
                detach _x;
                _x disableCollisionWith _veh;   
            } count (_this select 1);
            _time = time + 5;
            waitUntil {time > _time};
            {
                if (!isNull _x) then {deleteVehicle _x};
            } count (_this select 1);
        };
    };
	
params ["_classNameToDrop", "_positionToDrop"];
_positionToDrop set [2, 1000];

private _vehicle = createvehicle [_classNameToDrop, _positionToDrop, [], 0, "NONE"];        
createVehicleCrew _vehicle;

[_vehicle] remoteExec ["grad_hostomel_smoke"];

_vehicle call KK_fnc_paraDrop;
